...
func ScanWorkflows(sourceDir string) ([]*Action, error) {
    ...
    for _, file := range files {
        filePath := path.Join(workflowsDir, file.Name())
        if file.IsDir() {
            fmt.Println(file.Name(), "is Dir, skip.")
        } else {
...
func parseImageFile(imageFile string) (string, int, error) {
    ...
    if err != nil {
        fmt.Println("open file", file, err.Error())
        return "", 0, err
    }
    defer file.Close()
    ...
    if err != nil {
        fmt.Println("read file", file, err.Error())
        return "", 0, err
    }
        ...
        for name, job := range action.Jobs {
            for _, step := range job.Steps {
                if strings.HasPrefix(step.Uses, "x-actions/python3-cisctl") {
                    ...
                    rawSrcImageListURLArray := strings.Split(rawSrcImageListURL, "/")
                    imageFileName := rawSrcImageListURLArray[len(rawSrcImageListURLArray)-1]
